...
- **Ref 机制** - 稳定的元素引用（`@e1`, `@e2`），可靠的自动化操作
- **网络拦截** - Mock API 响应、请求录制、流量模拟
...
- **视觉回归** - 截图对比与基线管理
- **交互测试** - 键盘导航、拖放、响应式测试
- **流程图测试** - 测试 @xyflow/react 图表（节点、连线、布局）
- **虚拟列表测试** - 大数据量虚拟滚动和无限加载测试
- **Monaco 编辑器测试** - 代码编辑器测试（语法、补全、导航）
- **时间线测试** - 异步时间线和甘特图组件测试

### 录制与报告
- **屏幕录制** - 截图、视频录制、GIF 生成、标注
- **测试可视化** - HTML 报告，包含步骤时间线和截图
- **性能基准测试** - 计时、内存、资源使用量测量

### 高级功能
- **会话管理** - 多会话并行测试
- **流式服务器** - WebSocket 实时预览
- **Daemon 模式** - 持久化进程，加速连续测试
- **Tauri 原生对话框** - 测试文件打开/保存对话框
